 ...
 */
public class DetalheCalca extends DetalheProdutoAbstrato implements ActionListener {
    ...
    /**
     * Mostra janela com elementos para preencher dados de calca
     * @param opcao  referente a opcao selecionada
     * @param controleDados   importa valores de ControleDados
     * @param posicao referente a posicao de calca
     */
    public DetalheCalca(int opcao, ControleDados controleDados, int posicao) {
        super(new JFrame());
        ...
        labelModelo = new JLabel("Modelo Calca: ");
        botaoExcluir = new JButton("Excluir");
        botaoSalvar = new JButton("Salvar");

        // Mostrar dados cadastrados
        if (opcao == 2) {
            valorID = new JTextField(String.valueOf(dados.getCalcas()[posicao].getIdProduto()), 200);
            valorNome = new JTextField(dados.getCalcas()[posicao].getNome(), 200);
            valorMarca = new JTextField(dados.getCalcas()[posicao].getMarca(), 200);
            valorPreco = new JTextField(String.valueOf(dados.getCalcas()[posicao].getPreco()), 200);
            valorDepartamento = new JTextField(dados.getCalcas()[posicao].getDepartamento(), 200);
            valorDescricao = new JTextField(dados.getCalcas()[posicao].getDescricao(), 200);
            valorCor = new JTextField(dados.getCalcas()[posicao].getCor(), 200);
            valorTamanho = new JTextField(String.valueOf(dados.getCalcas()[posicao].getTamanho()), 200);
            valorModelo = new JTextField(dados.getCalcas()[posicao].getModeloCalca(), 200);
     ...
     * As acoes tratadas sao salvar ou excluir calca da lista
     * @param e evento de acao do botao
     */
    // chama funcoeses la em controle de dados
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == botaoSalvar) {
            try {
                ...
                if (res) {
                    Utilitario.mensagemSucessoCadastro(this.janela);
                } else
        ...
        if (src == botaoExcluir) {
            ...
            if (opcao == 2) {
                ...
                if (res)
                    Utilitario.mensagemSucessoExclusao(this.janela);
                else
    ...
    public boolean addItemSelecionadoCarrinho() {
        return dados.getCarrinho().addItem(dados.getCalcas()[posicao]);
    }
}
