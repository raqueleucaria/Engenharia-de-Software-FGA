...
void solve()
{
    ...
    for (int i = 0; i < 91; i++)
        if (ps[i].value)  ps[pn++] = ps[i];
    for (int i = 0; i < 901; i++)
        if (ss[i].value)  ss[sn++] = ss[i];
    for (int i = 0; i < 9001; i++)
        if (dfs[i].value)  dfs[dfn++] = dfs[i];
    for (int i = 0; i < 90001; i++)
        if (des[i].value)   des[den++] = des[i];
    ...
    if ((double)ps[0].value / totalp >= .51) printf("%d\n", ps[0].key);
    else printf("Segundo turno\n");

    for (int i = 0; i < s; i++) printf("%d%c", ss[i].key, " \n"[i == s - 1]);
    for (int i = 0; i < f; i++) printf("%d%c", dfs[i].key, " \n"[i == f - 1]);
    for (int i = 0; i < e; i++) printf("%d%c", des[i].key, " \n"[i == e - 1]);
}
