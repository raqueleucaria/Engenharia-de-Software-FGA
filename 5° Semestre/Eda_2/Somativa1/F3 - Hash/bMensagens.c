...
unsigned hashu(const char *x) {
    ...
    for (int i = 0; x[i] != '\0'; i++) h = (h * MULT_HASH_VALUE + x[i]) % HT_SIZE;
    return h;
}
...
HashTable HT_init() {
    ...
    for (unsigned i = 0; i < HT_SIZE; i++) ht.v[i] = NULL;
    return ht;
}
...
int HT_insert(HashTable *ht, Item x) {
    ...
    while (curr->next && !eq(curr->item, x)) curr = curr->next;
    if (eq(curr->item, x)) return 0;
...
int HT_remove(HashTable *ht, key_t k) {
    ...
    if (eq_k(curr->item, k)) {
        ...
        while (curr->next && !eq_k(curr->next->item, k)) curr = curr->next;
...
int HT_delete(HashTable *ht, Item x) {
    ...
    if (eq(curr->item, x)){
        ...
        while (curr->next && !eq(curr->next->item, x)) curr = curr->next;
...
LinkedItem *HT_search(HashTable ht, key_t k) {
    ...
    while (curr)
    {
        if (eq_k(curr->item, k)) return curr;
        curr = curr->next;
    }
    return NULL;
}
